...
public class TrainMapQuestion {
    ...
    private static class TrainMap {
        ...
        public List<Station> shortestPath(String from, String to) {
            ...
            q.offer(stations.get(from));
            Map<Station, List<Station>> path = new HashMap<>();
            path.put(q.peek(), new ArrayList<>());
            path.get(q.peek()).add(q.peek());

            Set<Station> visited = new HashSet<>();

            while (!q.isEmpty()) {
                Station s = q.poll();
                for (Station c : s.neighbours) {
                    if (!visited.contains(c)) {
                        visited.add(c);
                        path.put(c, new ArrayList<>(path.get(s)));
                        path.get(c).add(c);
                        q.offer(c);
                    }
                }
            ...
            System.out.println(path.get(stations.get(to)));
            return path.get(stations.get(to));
        }
    ...
    public static boolean doTestsPass() {
        ...
        trainMap.connectStations(trainMap.getStation("King's Cross St Pancras"), trainMap.getStation("Russel Square"))
                .connectStations(trainMap.getStation("King's Cross St Pancras"), trainMap.getStation("Angel"))
